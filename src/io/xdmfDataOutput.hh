...
#ifndef XDMF_DATA_OUTPUT_HH
#define XDMF_DATA_OUTPUT_HH
...
template <typename T>
void ParallelXdmfDataWriter3D::writeDataField(
    MultiBlock3D &multiBlock, const std::string &field_name)
{
    ...
    if (global::mpi().isMainProcessor()) {
        ...
        for (; it != bulks.end(); ++it) {
            ...
            (*fhandle) << "\t\t\t\t\t<DataItem DataType = \"Float\" ItemType = \"Uniform\" "
                          "Precision = \""
                       << sizeof(T) << "\" Dimensions = \"" << total_size << "\" Format = \"HDF\">"
                       << h5_fname << ":/" << set_name << "</DataItem>\n";
            (*fhandle) << "\t\t\t\t</DataItem>\n";
            (*fhandle) << "\t\t\t</Attribute>\n";
            (*fhandle) << "\t\t</Grid>\n";
...
template <typename T>
void ParallelXdmfDataWriter2D::writeDataField(
    MultiBlock2D &multiBlock, const std::string &field_name)
{
    std::vector<plint> offset;
    std::vector<plint> myBlockIds;
    std::vector<std::vector<char>> data;
    ...
    if (global::mpi().isMainProcessor()) {
        ...
        for (; it != bulks.end(); ++it) {
            ...
            (*fhandle) << "\t\t\t<Topology Dimensions = \"" << xn + 1 << " " << yn + 1 << " "
                       << "\" Type = \"2DCoRectMesh\" />\n";
            (*fhandle) << "\t\t\t<Geometry Origin = \"\" Type = \"ORIGIN_DXDYDZ\">\n";
            (*fhandle) << "\t\t\t\t<DataItem DataType = \"Float\" Dimensions = \"2\" Format = "
                          "\"XML\" >"
                       << dx << " " << dy << " " << "</DataItem>\n";
            (*fhandle) << "\t\t\t\t<DataItem DataType = \"Float\" Dimensions = \"2\" Format = "
                          "\"XML\" >1 1</DataItem>\n";
            ...
            (*fhandle) << "\t\t\t\t<DataItem ItemType = \"HyperSlab\" Dimensions = \"" << xn << " "
                       ...
                       << "\" Type = \"HyperSlab\" Format = \"XML\"  DataType = \"Float\"  "
                          "Precision = \""
                       << sizeof(T) << "\">\n";
            (*fhandle) << "\t\t\t\t\t<DataItem  Dimensions = \"2\" Format = \"XML\">" << start
                       << "  1  " << size << "</DataItem>\n";
            (*fhandle) << "\t\t\t\t\t<DataItem DataType = \"Float\" ItemType = \"Uniform\" "
                          "Precision = \""
                       << sizeof(T) << "\" Dimensions = \"" << total_size << "\" Format = \"HDF\">"
